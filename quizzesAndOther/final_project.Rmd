...
Because the assignment calls for cross-validation, we'll create a validation set with 25% of observations.
...
## Feature selection
Next, principle component analysis will be applied in order to reduce the number of predictors.

```{r, message = FALSE}
# preProc <- preProcess(df[, -length(df)], method= "pca", thresh = .95)
# preProc
```

Twenty-five components capture 95% of the variance. So, now we'll incorporate the principle components into the training and validation sets.
```{r, message = FALSE}
trainPC <- predict(preProc, df[, -length(df)])

#validPC <- predict(preProc, df.v[,-length(df.v)])
```

## Building  and testing model

The first type of model to try will be random forest. 
